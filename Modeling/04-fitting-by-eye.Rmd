...
Using today's technology, Prof. Stan Wagon of Macalester College investigated the accuracty Newton's "Law." 
@fig-Fun-4-intro-1 shows some of Wagon's data from experiments with cooling water. He poured boiling water from a kettle into an empty room-temperature mug (26 degrees C) and measured the temperature of the water over the next few hours.
...
This chapter shows simple techniques to translate data, such as in @fig-stans-data, into a quantitative model assembled from the pattern-book functions. We'll do this by making a handful of measurements of the shape of the pattern shown by the data and translating those measurements into the parameters introduced in @sec-assembling.

In each case, the first step is to select a pattern-book function that has roughly the same shape as the data.
...
It seems fair to say that the data in @fig-stans-data don't match any of the pattern-book shapes in #tbl-pattern-book-shapes.
...
#| label: fig-four-variations
#| fig-cap: "Four variations of the exponential function." 
#| column: page-right
Pa <- slice_plot(exp(x) ~ x, domain(x=c(-2,2))) %>%
  gf_refine(theme_void()) %>%
  gf_theme(plot.title = element_text(size=16)) %>%
  gf_labs(subtitle="A. Exponential function") 
  
Pb <- slice_plot(exp(-x) ~ x, domain(x=c(-2,2))) %>%
  gf_refine(theme_void()) %>%
    gf_theme(plot.title = element_text(size=16)) %>%
  gf_labs(subtitle="B. Flipped right-for-left")

Pc <- slice_plot(-exp(x) ~ x, domain(x=c(-2,2))) %>%
  gf_refine(theme_void()) %>%
    gf_theme(plot.title = element_text(size=16)) %>%
  gf_labs(subtitle="C. Flipped top-for-bottom")
Pd <- slice_plot(-exp(-x) ~ x, domain(x=c(-2,2))) %>%
  gf_refine(theme_void()) %>%
    gf_theme(plot.title = element_text(size=16)) %>%
  gf_labs(subtitle="D. Flipped both right-for-left and top-for-bottom")
...
```{r echo=FALSE}
#| label: fig-Fun-4-a-3-1
#| fig-cap: "About four days of tide-level data from Providence, Rhode Island"
...
```{r echo=FALSE, fig.show="hold"}
#| label: fig-Fun-4-a-3-2
#| out-width: "50%"
#| fig-cap: "Zooming in on the start of the data (left) and on the last part of the data (right)."
gf_line(level ~ hour, 
...
```{r echo=FALSE}
#| label: fig-Fun-4-a-3-3
#| fig-cap: "The sinusoid fails to align with the timing of peaks and troughs."
...
```{r echo=FALSE}
#| label: fig-Fun-4-a-2-1
#| fig-cap: "The cooling-water data, repeated here for convenience. We've sketched in an exponential-like curve that matches the data pretty well."
set.seed(101)
gf_point(temp ~ time, data=CoolingWater %>% sample_n(20)) %>%
  gf_lims(y = c(20, NA)) %>%
  gf_smooth()
...
```{r echo=FALSE}
#| label: fig-Fun-4-a-2-2
#| fig-cap: "An exponential function that roughly aligns with the data."
set.seed(101)
gf_point(temp ~ time, data=CoolingWater %>% sample_n(20)) %>%
  gf_lims(y = c(20, NA)) %>%
...
```{r echo=FALSE, warning=FALSE}
#| label: fig-show-sin-params
#| fig-cap: "Baseline (blue), amplitude (red), and period (green) for the sinusoid."
...
```{r echo=FALSE, warning=FALSE} 
#| label: fig-show-exp-baseline
#| fig-cap: "The baseline for the exponential is the horizontal asymptote. There is no vertical asymptote."
