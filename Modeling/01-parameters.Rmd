...
The variety of shapes of the nine pattern-book functions means that, often, one or another will be suitable for the modeling situation in hand. [Marginal: Combining the functions to create a greater diversity of shapes is the subject of Chapter COMBINING]{.aside}
...
Pcases <- gf_point(confirmed ~ day, 
  ...
  gf_lims(y=c(0,20)) 
Pcases %>%
  slice_plot(exp(t) ~ x, domain(t=c(0,7)), 
...
#| fig-cap-location: margin
Pcases %>%
  slice_plot(.0573*exp(0.19*t) ~ t, interval(t=0:31), color="blue", label_x=0.5,
...
At the heart of how we are going to use the pattern-book functions to model the relationship between quantities is the idea of conversion between one scale and another.  Consider these everyday objects: a thermometer and a ruler.
...
In Section @sec-input-scaling, we will use the $ax + b$ form of scale conversion, to scale the **input** to pattern-book functions, but we could equally well have used $a(x-x_0)$.

In Section @sec-output-scaling we will introduce a second scale conversion function, for the **output** from pattern-book functions. That scaling will also be in the form of a straight-line function: $A x + B$. The use of the lower-case parameter names ($a$, $b$) versus the upper-case parameter names ($A$, $B$) will help us distinguish the two different uses for scale conversion, namely ***input scaling*** versus ***output scaling***.
...
`r insert_exercises("Modeling", "parameters"`)
