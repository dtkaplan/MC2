...
ii. Find the t-value where the horizontal line intersects the function graph. There's only one such intersection and that's at about $t=1.2$. 
...
We'll continue our iteration of better().
```{r}
...
Let's suppose you happened on a guess that is exactly right, that is $y = \sqrt{x}$. There's no way to improve on a guess that is exactly right, so the best better() can do is to return the guess unaltered. Indeed it does:
...
`r insert_calcZ_exercise("XX.XX", "2svBa5", "/Users/kaplan/MC2/Modeling/Exercises/snail-pitch-knife.Rmd", skill="Graphical zero finding")`
...
EXERCISE ON FINDING HOW LONG UNTIL radioactivity level decays to 1/100 of it's  current level. Write the function as 2^t^ and have them use log2() to calculate the result.
