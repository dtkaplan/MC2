...
Pbowl <- contour_plot(Bowl(x, y) ~ x & y, domain(x=-3:3, y=-3:3), skip=0) %>%
  gf_refine(coord_fixed())

Phill <- contour_plot(40-Bowl(x, y) ~ x & y, domain(x=-3:3, y=-3:3), skip=0) %>%
  gf_refine(coord_fixed()) 
...
#| out-width: "150%"
#| column: page-inset-right
Pa <- Pbowl %>% gf_labs(subtitle="Bowl")
Pb <- Phill %>% gf_labs(subtitle="Hill")

Pc <- Saddle$P1 %>%
  gf_refine(coord_fixed()) %>% gf_labs(subtitle="Saddle")
