...
Estimating $t_0$ can only be done once the period $P$ is known precisely. In practice, as you'll see in Chapter @sec-modeling-cycle, we use a numerical method called ***polishing*** to estimate both $P$ and $t_0$ at the same time.
...
As for finding the power law $f(x) \equiv A x^p$ that provides a good match to data, we'll need some additional tools to be introduced in Chapter @sec-magnitudes.
