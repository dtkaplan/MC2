...
@fig-water-exponential shows an exponential function fitted to the water-cooling data. The fitted function is stored as `Mod1_fun()` with the `1` identifying it as the first of possibly several models.
...
Refer to @fig-water-exponential, showing the measured temperature data along with the initial fitted exponential model. There are four places where the data systematically differ from the model: the model underestimates the temperature early in the process, then overestimates for a interval, returning later to an underestimation and finishing up with an over-estimation. 

In assessing a model, it is a good practice to calculate the *difference* between the observed data and the model value. @fig-water-residuals shows this difference, calculated simply by subtracting from each data point the model value at the corresponding time. In statistical nomenclature, such differences are called the "**residuals**" from the model.
...
Residuals versus time. The comparatively smooth structure indicates systematic deviations between the model and the data.
:::

The presence of *noise* in any measurement is to be expected. You can see such noise in @fig-water-residuals in the small, irregular trembling fluctuations. The large, smooth, slowly changing pattern in the residuals is the systematic part. The smooth variations are much larger in amplitude than the noise, a sign that we should consider them carefully.

This is a chance for the modeler to speculate on what might be causing the systematic deviations. A good place to start is with the largest residuals. In this case, that's at the beginning of the temperature recording. Here, the phenomenon we seek to explain is that the actual recorded temperatures fall much faster than the model. (This is perhaps clearer with reference to @fig-water-exponential.)
...
@fig-water-two-exps shows a much improved match between the model and the data. The residuals are only about one-tenth as large as those from the original model.

Depending on the purpose for which the modeling is being done, this might be the end of the story. The model is within a few tenths of a degree C from the data, good enough for purposes such as prediction. For Prof. Wagon, however, the purpose was to investigate how complete an explanation Newton's Law of Cooling is for the physical process. His conclusion was that the residuals in @fig-water-two-exps still show systematic patterns.

## Refine the data, refine the experimental technique

Often, the initial model building will suggest how new, informative data might be collected. It's impossible to generalize this to all situations, but in Prof. Wagon's work, two possibilities arise:
...
Prof. Wagon managed (2) by putting adding a small drop of oil right after the water was poured into the mug. This created a thin layer that hindered evaporation. His expectation was that cancelling this non-Newtonian process would make the Newton model a better fit to the new data, providing an estimate of the magnitude of the evaporation effect.  
...
Another important statistical topic is *causality*; reasoning about what causes what. Often, the modeler's understanding of how the system under study actually works can guide the choice of additional variables to include in a model.

Space precludes our including detailed consideration of the statistical phenomena in this book, although the techniques used are well within the capabilities of anyone who has completed this book through Block 3. 
...
Isaac Newton famously developed his universal theory of gravitation through examination of the orbits of planets. Planets close to the sun feel a strong tug of the Sun's gravity, farther-out planets have receive a weaker tug. Similarly, the motion of satellites around the Earth is determined mainly by the Earth's gravitational pull. Per Newton's theory, the acceleration due to gravity goes as the *inverse-square of distance* from the center of the Earth. Far-away satellites have long orbits, close-in satellites orbit with far lower orbital duration.

Planets orbits outside the Sun. Satellites orbit outside the Earth. Application of the inverse-square law to forces *inside* the Sun or *inside* the Earth are interpolation. But extrapolation, for instance using the inverse square law to model forces inside the Sun or Earth, produce catastrophically misleading models. In reality, gravitational force on the inside---as you might imagine in a tunnel going through the center of the Earth---increase linearly with distance from the center. (Newton's theory explains the mechanics of this.)
:::
...
Empirical models---sometimes deprecatingly called "curve fitting"---are common in many important areas: the social sciences and economics, psychology, clinical medicine, etc. A solid understanding of statistics greatly enhances the reliability of conclusions drawn from empirical models. 

Outside of physical law, geometry provides many examples of mechanistic connections between elements, for example calculating the force on an arrow being pulled back by the draw of a bow. Another important area of such mechanisms is the rates of chemical reactions. Using such mechanisms, known or expected, can add reliability to models.

## Polishing parameters {#sec-polishing-parameters}

In @sec-polishing-promise we referred to a way to improve parameters in order to better match patterns in data: polishing. Polishing can be carried out by software such as `fitModel()`. It's worthwhile because it is easy and mostly automatic. 

Polishing becomes harmful when the modeler misinterprets it as increasing the reliability of a model. Polishing can add a digit or two to a model parameter, but those digits may be irrelevant from a statistical perspective and they do not provide any warranty that the function being fitted is an appropriate approximation to the real-world situation. 
...
Despite Arrhenius's prescience, widespread understanding of greenhouse gas effects emerged only around 1990. Up until then, the possibility of concern was that the world might run out of oil and freeze the economy. Policy makers attempted to use past data to predict future oil. These were entirely empirical models: curve fitting. The curve selected was sensible: a sigmoidal function. The time until world oil was exhausted corresponded to the sigmoidal curve reaching it's upper horizontal asymptote.

@fig-peak-oil shows cumulative oil consumption globally as of each year, starting in 1900 and running through 2023. It's revealing to look at a fitted curve using data up to 1973. (In 1973 the world oil economy started to change rapidly, due to embargoes and oligopolistic behavior by leading oil producers.) The blue curve in @fig-peak-oil shows a sigmoidal function as might have been fitted in 1973.
