...
gf_line(level ~ hour, data = Zcalc::RI_tide) %>%
  ...
  gf_lims(y =c(0, 2))
```
We might call it quits with the model in Figure \@ref(fig:Fun-4-a-3-5). But once we have a pretty good model fit, it is easy to polish the parameter estimates, letting the computer do the tedious work of trying little tweaks to see if it can improve the fit.
...
askMC(
  prompt = "What is the range of the number of minutes until sunset over the whole 4-year period?",
    ...
    "0 to 48  months" = "that is  the *domain*. The *range* is along the vertical axis."
)
```
...
askMC(
  prompt = "Your `sunset()` function should be a pretty good match to the data except for one thing. What is that thing?",
    ...
    "The period of the `sunset()` function does not match the data." = "Did you use `sin(2*pi*Month/12)`? If so, the period should be right.",
    "+There is a horizontal time shift between `sunset()` and the data.+",
  inline = FALSE
)
```

We are going to fix the problem with `sunset()` by defining a *time offset* to use as a reference. For a sine function, a suitable time offset is the value along  the horizontal axis when the phenomenon being modeled crosses $C$ with  a positive slope. There are 4 such points along the horizontal axis readily identifiable in the data. (They may not be at an integer value of `Month`.)

```{r dd-07-QA4, echo=FALSE}
askMC(
  prompt = "Which of these is a suitable value for the time offset?",
    "0 months" = "that is  not a time when the data suggest that $C$ is being crossed.",
    "19 months" = "that is  a maximum, not a crossing  of $C$.",
    "21.5 months" = "that is  a crossing of $C$, but not one with  a positive slope.",
