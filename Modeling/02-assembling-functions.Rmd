...
Each of the functions is being scaled by a quantity: 3, 5, and -2 in this example. Then the scaled functions are added up. That's a linear combination; scale and add. 
...
[Note that neither the parameterized exponential or the parameterized sinusoid is a polynomial. A polynomial is always a linear combination of monomials. (As Section @sec-power-law-family, describes, monomials are the power-law functions $x^0$, $x_1$, $x_2$, and so on.)]{.aside} 
...
::: {.example data-latex=""}
Any one who lives far from the equator is familiar with the annual cycle of short winter days and long summer days. The magnitude of this cycle is a function of ***latitude***; the further away from the equator the larger the winter-summer day-length difference. 

A simple model accounts for length of daylight (in hours) as a function of latitude $L$ and the declination angle $\delta$ of the sun. 
...
On the summer solstice, the longest day of the year, it is $23.44^\circ$. On $day$, where midnight before January 1 is $doy=0$ and  is $doy=365.25$, the declination as a function of is a composition of $\sin()$ with the straight-line function ${\small\frac{2\pi}{365.25}} (doy - 9)$.
...
$\delta(doy) = 23.44 \sin\left({\small\frac{2\pi}{365.25}} (doy-9)\right)$]{.aside}

The output has units degrees.
...
$$\text{daylight}(L, day) \equiv {\small\frac{2}{15}} \arccos\left(-\tan(L)*\tan(\delta(doy))\right)\ .$$
