---
id: "ape-choose-closet"
created: "Sun Dec  5 16:15:49 2021"
global_id: "QXWRP8"
---


The figure shows a contour plot of an objective function $f(x,y)$ along with the gradient field of that function. 

```{r fg2-11, echo=FALSE, warning=FALSE}
f <- rfun( ~ x + y, seed=103)
f2 <- rfun( ~ x + y, seed=116)
dx_f <- D(f(x, y) ~ x)
dy_f <- D(f(x, y) ~ y)
dx_f2 <- D(f2(x, y) ~ x)
dy_f2 <- D(f2(x, y) ~ y)
soln1 <- integrateODE(dx ~ dx_f(x=x, y=y), 
                     dy ~ dy_f(x=x, y=y), 
                     x = 1, y = -3,
                     tdur = list(from=0, to=10, dt=0.1))
soln2 <- integrateODE(dx ~ -dx_f(x=x, y=y), 
                     dy ~ -dy_f(x=x, y=y), 
                     x = -2, y = 0.75,
                     tdur = list(from=0, to=10, dt=0.1))
soln3 <- integrateODE(dx ~ -dx_f2(x=x, y=y), 
                     dy~ -dy_f2(x=x, y=y), 
                     x = 3, y = 3,
                     tdur = list(from=0, to=10, dt=0.1))

Common <- contour_plot(f(x, y) ~ x + y, bounds(x=c(-4, 4), y=c(-4, 4))) %>%
  gradient_plot(f(x, y) ~ x + y) 
P1 <- Common%>% traj_plot(y(t) ~ x(t), soln1, color="dodgerblue") %>%
  gf_labs(title="Plot 1")
P2 <- Common%>%   traj_plot(y(t) ~ x(t), soln2, color="orange3") %>%
  gf_labs(title="Plot 2")
P3 <- Common%>%   traj_plot(y(t) ~ x(t), soln3, color="magenta") %>%
  gf_labs(title="Plot 3")
P1
```

Is the path a  gradient **ascent**, a gradient **descent**, or neither?


