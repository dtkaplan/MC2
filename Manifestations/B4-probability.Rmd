...
Showing a probability density function for the dart distance in two modes: 1) an ordinary function graph and 2) the density of ink.
:::

::: {#thm-jar-of-money style="display: none;"}
:::
::: {.callout-note}
## @thm-jar-of-money Winning a competition
...
![](www/jar-coins.png){align="center"}
...
The entry form for the money-in-the-jar contest.
:::
...
@fig-contest-entries shows the contest entries from three competitors. 
...
P2 <- Money_graph |> gf_labs(title="Name: Louisa") |>
  ...
  scale_y_continuous(limits=c(0,7), breaks=0)
...
Three contestants' contest entries.
:::
...
::: {#fig-three-density-funs .column-page-right}
```{r echo=FALSE}
#| layout-ncol: 3
#| fig-subcap: 
#|   - "Gaussian density"
...
Three probability density functions that are often used in applied work.
:::
...
::: {#try-gaussian-integrate style="display: none;"}
:::
::: {.callout-important}
## @try-gaussian-integrate Computing expected value
...
::: {#try-gaussian-variance style="display: none;"}
:::
::: {.callout-important}
## @try-gaussian-variance Computing variance
...
::: {#thm-who-won style="display: none;"}
:::
::: {.callout-note}
## @thm-who-won Who won? (Bayesian style)
...
::: {#thm-likelihood-of-illness style="display: none;"}
:::
::: {.callout-note icon=false}
## @thm-likelihood-of-illness Likelihood of illness
...
Suppose our first observations are the results of screening tests on ten randomly selected individuals, as in @tbl-test-outcome. `r set.seed(101)` 
...
Test outcomes on ten patients. There were three $+$ tests. 
:::
...
Posteriors (blue) consider only the first two screening tests, one $+$ and one $-$, for each of the priors in @fig-beta-shapes. The prior itself is drawn in gray.
:::
...
More data might help things! We can continue on in this way through all ten rows of the data to get the posterior distribution after all 10 test results have been incorporated, as in @fig-after-10.
...
Posteriors (blue) after the first ten rows of data, for each of the priors in @fig-beta-shapes. The prior itself is drawn in gray.
:::
...
Posteriors (blue) after 100 subjects have been screen, with 30 $+$ results.
:::
