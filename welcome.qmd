...
For convenience, we placed "Active R Chunks" within the text. Often, as in @lst-in-preface, these will be presented with working R/mosaic commands already included. Just press "Run Code" to have the commands evaluated.

::: {#lst-in-preface}
```{webr-r}
#| label: lst-in-preface
date()
```
:::
...
This book, as well as the exercises in the MOSAIC Calculus *Workbook*, provide immediate access to R/mosaic software via interactive elements such as @lst-in-preface that run in your browser.
