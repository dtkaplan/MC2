...
contour_plot(f(x, y) ~ x + y, bounds(x=c(-5,5), y=c(-5,5)), filled=FALSE) %>%
  ...
  gf_segment(-5 + 5 ~ 3 + -1, color = "dodgerblue",linetype="dotted", size=2, alpha=.5) |> #
  gf_segment(-5 + 0 ~ -5 + 5, color = "orange3", , linetype="dashed", size=2, alpha = .5) |> #1
  gf_segment(-2 + -5  ~ -5 + 5, color = "yellow3", size=2, linetype="dotdash", alpha=.5) |> # 4
  gf_segment(5 + -5 ~ 4 + -2.5, color= "gray", size=2, linetype="longdash", alpha = .5) # 2
```


```{r echo=FALSE, warning=FALSE}
dom <- bounds(t=c(0,1))
pA <- slice_plot( f(-5 + 10*t, 4) ~ t, dom, label_text = "slice (3)", label_x=.5) |> #black
  slice_plot( f(-5 + 10 *t, -5 + 5*t) ~ t, label_text = "slice (1)", label_x=.75) # tan
pB <- slice_plot(f(-5 + 10*t, -2 - t*3) ~ t, dom, label_text = "slice (4)", label_x = 3 ) |> #yellow
