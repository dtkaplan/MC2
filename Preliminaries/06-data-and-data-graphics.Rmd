...
THE ABOVE IS AN EXAMPLE OF HOW TO CAPTION and label a table.
...
::: {#tbl-births-deaths layout-ncol=2}

males | females | year
------|---------|------
5114  | 4853    | 1665

: "Christened": the number of births {#tbl-christened}

males | females | year
------|---------|------
48569 | 48737   | 1665

: "Buried": the number of deaths {#tbl-buried}

Two tables from near the bottom of the Bill of Mortality, in a contemporary format. In the Christened and Buried tables, the case is "one year." (We're imagining that the data from other years would go in additional rows.) Summaries such as the "In all" for the Christened and Plague tables would not be included in a moern format, since they can be computed from other data in the table. Such summaries are easily computed as needed using appropriate computer database commands. 

:::

::: {.column-margin}

You might wonder why the Christened and Buried tables aren't put together in a single table, perhaps something like

           | males | females | year 
:----------|------:|--------:|------
Christened | 5114  | 4853    | 1665
Buried     |48569  | 48737   | 1665

One the important topics in the field of ***data science*** is the effective organization of data. It's beyond the scope of this book why the table above is not a professional form for data organization. (A hint is the lack of a name for the first column; it's hard to say what is being captured by that column.)
:::

A modern organization for the data being presented in the Bill of Mortality go back to the raw records that were collected in the field, stacking them into just two tables: Deaths and Births.
...
: An imagined re-organization of the data that went into the Bill of Mortality report {#tbl-raw-deaths}
...
```{r echo=FALSE}
#| label: tbl-engine-table
#| fig-cap: "Various attributes of internal combustion engines, from the very small to the very large."
set.seed(104)
