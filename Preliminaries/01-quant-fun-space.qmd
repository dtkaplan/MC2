...
A concise definition^[Source: Oxford Languages] of "mathematics" is "the abstract science of number, quantity, and space." The chapter title, however, doesn't include the word "number." Why not? The answer lies in another word in the definition: "abstract." 

To be abstract means to be "based on general ideas and not on any particular person, thing, or situation." Numbers are abstract; they can be used to represent all sorts of things, sheep, blood pressure, age, and so on without end. The mechanics of working with numbers is the same regardless of the kind of thing involved. In other words, abstract.

This book is about *applied mathematics*: the topics and methods of mathematics that are particularly useful in representing---that is, "*modeling*"---particular objects or situations. The topics and methods of applied mathematics are abstract in the sense that they can be usefully applied to a huge range of situations. But the topics are carefully selected based on experience to be particulary useful in modeling situations and drawing conclusions from those models.
...
As you know, a spreadsheet is a device for calculating with numbers. This book is about another kind of calculating device. The root of the word "calculate" is "calculus," Latin for "stone" or "pepple." Thousands of years before spreadsheets were invented, people were using tables with divots holding pepples. Moving the pepples around was the mechanism for calculation. An "abacus" is a particularly clever re-arrangement that replaces the pepples with beads on a stick. For the last 250 years, however, "Calculus" has come to refer to a set of mathematical concepts and techniques that emerged as part of the Enlightenment and proved spectacularly useful for creating and manipulating mathematical models in physics, chemistry, engineering, and a host of other fields such as economics.{.aside}(We will use capital letters for the mathematical kind of calculus: Calculus rather than calculus.) 
...
#| fig-cap: A number line drawn in the conventional style suitable for representing temperatures near freezing. 
#| column: page-right
...
A more general way for representing pairs of quantities is the **coordinate plane**, which @fig-coordinate-plane shows in the mathematics-text style.  (In @sec-graphs-and-graphics, we will switch to another style more commonly used across disciplines.) 
...
Thinking in terms of movement, it's easier to construct a depiction of even four-, five-, and higher-dimensional space. @fig-robot-youtube shows the movement of a robotic hand with six degrees of freedom:
...
There can be multiple ways of forming coordinates for the same space. In terms of the mechanism for the robotic hand, it's configuration can be specified by the six degrees of freedom enumerated in @lst-six-degrees. But from the point of view of a person training the robot, it might be more convenient to think about the configuration in terms of the six quantities given in @lst-six-degrees-2. 
