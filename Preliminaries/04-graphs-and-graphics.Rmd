...
@rcd-first-graph shows an R/mosaic graphics command that produces a *graphic* showing the *graph* of a function.:
...
The R/mosaic command you will use to graph a mathematical function is `slice_plot()`. (We will get into the reasons for this name later.) `slice_plot()` takes **two** arguments.

1. a tilde expression of the same sort you saw in @sec-computing-with-r. Examples are

    - `sin(x) ~ x`
    - `3 * y + b  ~ y`
    
2. the graphical domain, which you will write using the `domain()` command, as in
    - `domain(x = 0:5)`
    - `domain(y = -3:3)`
    
    EXPLAIN ABOUT THE COLON and the equal sign.


For reading questions: 

This command is invalid: `slice_plot(f, domain(x = 1:10))` where `f` is the name of a function. What's wrong with it?

no parentheses: that's true but it's hardly the only problem.
not a tilde expression
f hasn't been defined. true, but even if it had been defined, the command would be invalid.
