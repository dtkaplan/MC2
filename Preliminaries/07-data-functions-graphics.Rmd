...
```{webr-r}
stroke_model <- fitModel(stroke ~ A*bore^b, 
                         data = Engines)
gf_point(stroke ~ bore, data = Engines) |>
  slice_plot(stroke_model(bore) ~ bore, color="blue")
```
...
<!-- MYSTERY ... The following command works in the console, but not in a code chunk -->

```{r eval=FALSE, results="hide"} 
slice_plot(stroke_model(bore) ~ bore, domain(bore=0:1000))
```

[IN DRAFT: Resolve bug here and in next chunk.]{.aside}
...
There is an important pattern shared by all these tasks. In the tilde expression, the output is on the left-hand side of [~]{style="font-size:25pt;"} while the inputs are on the right-hand side. 

output [~]{style="font-size:25pt;"} input(s)
...
response var. [~]{style="font-size:25pt;"} explanatory var(s)
