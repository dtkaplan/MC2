...
#| column: column-page-inset-right
#| out-height: 4
#| out-width: 8
Pts <- tibble(
  n = (-2):3,
...
\strut t}$ | $1/2$
Animal lifespan (in the wild) versus body mass^[This relationship is true when comparing *species*. Individual-to-individual variation within a species does not follow this pattern.] | $L(M) = a M^{0.25}$ | 0.25
...
The logarithms, which we'll return to in Chapter `r Chaps$magnitudes` are the ***inverse*** function of the exponential: Rule (v).
...
```{r drill-Quiz-1-11, echo=FALSE, results='markup'}
askMC(
  prompt =  "By how much should you increase the input to the exponential function in order to produce a doubling of the output?",
  "0.5",
...
```{r drill-Quiz-1-12, echo=FALSE, results='markup'}
askMC(
  prompt =  "By how much should you increase the input to the exponential function in order to produce a quadrupling of the output?",
  "0.5",
  "0.693" = "This is called the doubling time.",
  "1",
...
```{r drill-Quiz-1-13, echo=FALSE, results='markup'}
askMC(
  prompt =  "By how much should you **decrease** the input to the exponential function in order to cut the output value by a factor of one-half?",
  "0.5",
