...
In its simplest use, `makeFun()` takes only one argument. That argument must be in a form called a ***tilde expression***. This name comes from the character tilde ([~]{style="font-size: 250%;"}) in the middle. On the right-hand side of the tilde goes the name of the input: `~ t`. On the left-hand side is the R expression for the formula to be used, written in terms of the input name and whatever parameters are used. For instance, @lst-h-with-params defines a parameterized version of $h()$. 
