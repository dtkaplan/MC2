...
Section @sec-totaling-bits showed some ways to visualize the construction of an $F(t)$ by accumulating short segments of slope. The idea is that we know $f(t)$ which tells us, at any instant, the slope of $F(t)$. So, in drawing a graph of $F(t)$, we put our pen to paper at some input value $t_0$ and then move forward in $t$, setting the instantaneous slope of our curve according to $f(t)$.
