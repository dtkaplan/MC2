...
Whenever we know a function $f(x)$ and have chosen a size for $dx$ we can draw a graph of $f(x)$ in the form shown in @fig-F-blank-axes2. We're drawing it in this unusual way because we want the graphics frame to be all ready for drawing the graph of $F(x)$ in the normal fashion after we have figured out what $F(x)$ results from accumulating/summing-up all the little $f(x) dx$ segments. When we write $\large\int$ in the notation $$\large \int f(x)\, dx$$ we mean, "sum up all the $f(x) dx$ segments."

Let's now consider how to "sum up all the segments." We'll start in @fig-F-232 with an example where we already know $F(x)$. That way, we can see of our sum of the $f(x) dx$ segments really does reconstruct $F(x)$. 
...
Start by reading off the function value from the last point in the left-most segment. That's been marked in @fig-F-232 with a blue dot. The function value at that dot is 7.072. 
...
Now read off the new value at the end of the second segment, it's 4.198. Add this amount to the third segment as in @fig-F-seg-2(bottom).
...
3. On each of the little chunks, draw in $f(x)$. Since $f(x)$ is the *slope* of $F(x)$, we will draw $f(x)$ for any given chunk as a short line segment of that slope over the chunk, as in @fig-F-blank-axes2. We'll write these little bits, each of which is very nearly a straight-line, as $\large\color{blue}{f(x) dx}$. 
