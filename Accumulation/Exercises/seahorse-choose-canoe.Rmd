...
askMC(
  ...
  -3.2, "-1.4"="Count the boxes in the interval", "0"="Count the boxes in the interval", "2.5"="Count the boxes in the interval", "3.3" = "that is  a pretty good count of boxes, but still not the right answer.", 6.1, right_one = -3.2,
...
```{r fig.height=2}
slice_plot(F(x) - F(4) ~ x, domain(x=0:10)) %>%
  gf_labs(title = "(A)")
slice_plot(F(4) - F(x) ~ x, domain(x=0:10)) %>%
  gf_labs(title = "(B)")
slice_plot(F(6) - F(x) ~ x, domain(x=0:10)) %>%
  gf_labs(title = "(C)")
slice_plot(abs(F(4) - F(x)) ~ x, domain(x=0:10)) %>%
  gf_labs(title = "(D)")
```
