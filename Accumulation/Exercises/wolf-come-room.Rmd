...
```{r DLMF2, echo=FALSE, results="markup"}
askMC(
  "Use Section 4.10 of DLMF to find $\\int \\frac{1}{e^{3 t} + 5}dt$",
  "+$\\frac{1}{15}(3t - \\ln(e^{3t} + 5))$+",
  "There's no such function listed in Section 4.10." = "Although DLFM wrote the function using the input name $z$, using $t$ instead is perfectly legitimate.",
  "$\\frac{1}{15}(5 t - \\ln(e^{5t} + 3))$" = "Looks like you mixed up the $a$ and $b$."
)
```

```{r DLMF3, echo=FALSE, results="markup"}
askMC(
...
```{r DLMF4, echo=FALSE, results="markup"}
askMC(
  ...
  "There's no $\\theta$ in section 4.26" = "The authors didn't use $\\theta$ as the name of the input, but $\\theta$ (\"theta\") is every bit as good a name to use as $x$ or $t$."
)
```
