...
It's not quite the case that $G(x)$ is the stock on the truck. to know that, we would have to know how many cones were on the truck when it started out. All we know for certain is that the flow of cones onto the truck (with respect to road position $x$) is the derivative of $G(x)$ w.r.t. $x$. That is, the flow onto the truck is $\partial_x G(x) = g(x)$.
...
3. $\int_a^y g(x) dx$ can be confusing. We're used to seeing a constant for the upper bound of integration, but here we have a name. It's easier to see what's going on if we express $\int_a^y g(x)dx$ in terms of an anti-derivative function $G(x)$:
