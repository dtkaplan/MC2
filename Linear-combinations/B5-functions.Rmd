...
Figure \@ref(fig:cello-seg) (top) shows the waveform of a note played on a cello. The note lasts about 1 second. The bottom panel zooms in on the waveform, showing 82 ms (that is, 0.082 s).
...
In modeling the cello waveform as a linear combination of sinusoids, the frequencies we use ought to respect the period of the cello vibration. Figure \@ref(fig:cello-seg2) shows the original waveform as well as the projection of the waveform onto a sinusoid with a frequency of 109.76 Hz. The figure also shows the residual from the projection, which is simply the original waveform minus the projected version.

```{r cello-seg2, echo=FALSE, fig-cap: "Top: The cello waveform and its projection onto a sinusoid with frequency $\\omega = 109.76$ Hz. Bottom: The residual from the projection."}
omega <- 109.76
Pts <- Cello_seg %>%
...
gridExtra::grid.arrange(P1, P2, nrow=2)
```
The sinusoid with $\omega = 109.76$ is not the only one that will repeat every 0.0091 s. So will a sinusoid with frequency $2\omega = 219.52$, one with frequency $3\omega = 329.28$ and so on. These multiples of $\omega$ are called the ***harmonics*** of that frequency. In Figure \@ref(fig:cello-build-up) (top) the cello waveform is projected onto $\omega$ and its first harmonic $2\omega$. In the middle panel, the projection is made onto $\omega$ and its first three harmonics. In the bottom panel, the projection is onto $\omega$ and its first eight harmonics.

```{r cello-build-up, echo=FALSE}
...
The decomposition into sinusoids offers another way of describing the cello waveform: the ***frequency domain***. In the frequency domain, we report the amplitude and phase of the projection onto each frequency, plotting that versus frequency. Figure \@ref(fig:cello-freq) shows the waveform in the frequency domain.

```{r cello-freq, echo=FALSE, fig-cap: "The frequency domain description of the cello waveform."}
n <- 51
omega <- 109.76
...
From the amplitude graph in Figure \@ref(fig:cello-freq), you can see that only a handful of frequencies account for almost all of the signal. Thus, the frequency domain representation is in many ways much more simple and compact than the time domain representation.
...
`r insert_calcZ_exercise("46.03", "uKOO7K", "Exercises/pony-stand-bottle.Rmd")`

`r insert_calcZ_exercise("46.05", "sNEKcy", "Exercises/walnut-bet-pants.Rmd")`
