...
THIS MATH MARKUP does not COMPILE TO PDF
...
A stickler for details might point out tht the "direction of the sun" has an upward component. Common sense will guide you to walk in the direction of the Sun as ***projected onto*** Earth's surface. Section @sec-projection-residual deals with projections of vectors. 
...
We will start with the idea that a vector is a column of numbers.
...
TAB <- if (knitr::is_html_output()) {
  ...
  Znotes::and_so_on_latex(CW, top=5, bottom=5)
}
TAB 
```
...
$$ 
A more concise way to write this collects the vectors with the values for the scalars into a matrix, which we will call $$\mathit{X} \equiv \left[\begin{array}{rrr}3 & 0 & -1\\1 & 2 & 0\end{array}\right]\ .$$ 
...
```{r child="CalcPlot3D/spaces-A.Rmd", eval=knitr::is_html_output()}
```
...
#| label: two-vecs
#| out-width: "1%"
#| fig-cap: "Two vectors $\\color{blue}{\\vec{u}}$ and $\\color{magenta}{\\vec{v}}$ embedded in 3-dimensional space. The subspace spanned by each vector, individually, is shown as a line. NEED TO PROVIDE PRINTABLE LINK FOR PDF version"
if (knitr::is_html_output()) {
  knitr::include_graphics("www/tiny-blank.png")
} else {
  # knitr::include_graphics("www/two-vecs.png")
}
```
...
```{r child="CalcPlot3D/spaces-C.Rmd", eval=knitr::is_html_output()}
```

```{r echo=FALSE}
#| label: fig-two-vecs-planes
#| out-width: "1%"
#| fig-cap: "Two vectors $\\vec{u}$ and $\\vec{w}$. The subspace spanned by two vectors is a plane, shown as a gray surface. NEED TO PROVIDE LINK for PDF version"
knitr::include_graphics("www/tiny-blank.png")
```
