...
A stickler for details might point out that the "direction of the sun" has an upward component. Common sense dictates that the walk is in the direction of the Sun as ***projected onto*** Earth's surface. [Section @sec-projection-residual] deals with projections of vectors. 

## Functions as vectors

Since calculus is about functions, one naturally expects a chapter on vectors in a calculus textbook to make a connection between functions and vectors. Recall from [Section @sec-functions-as-data] (entitled "Functions and data")  the idea of representing a function as a ***table*** of inputs and the corresponding outputs. 
...
```{r echo=FALSE}
cap_fig_two_vecs <- "Two vectors $\\color{blue}{\\vec{u}}$ and $\\color{magenta}{\\vec{v}}$ embedded in 3-dimensional space. The subspace spanned by an individual vector is shown as a line."
if (knitr::is_latex_output()) cap_fig_two_vecs <- paste(cap_fig_two_vecs, 
"Go to <https://dtkaplan.github.io/MC2/www/spaces-A.html> for an interactive, rotatable version.")
```


```{r echo=FALSE, out.width=ifelse(knitr::is_html_output(), "1%", "50%"), fig.cap=cap_fig_two_vecs}
#| label: fig-two-vecs
...
```{r echo=FALSE}
cap_fig_two_vecs_planes <- "Two vectors $\\vec{u}$ and $\\vec{w}$. The subspace spanned by two vectors is a plane, shown here as a gray surface." 
if (knitr::is_latex_output()) cap_fig_two_vecs_planes <- paste(cap_fig_two_vecs_planes, 
"Go to <https://dtkaplan.github.io/MC2/www/spaces-C.html> for an interactive, rotatable version.")
```
