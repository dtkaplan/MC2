...
Now we will confirm that `b_on_A` really is the projection of `b` onto `A`.  The strategy is to construct the residual from the projection.
```{r}
resid <- b - b_on_A
```
...
r %dot% v2
```
Note that we defined $\vec{r} = \vec{b} - \hat{\mathbf{b}}$, so it is guaranteed that $\vec{r} + \hat{\mathbf{b}}$ will equal $\vec{b}$.
