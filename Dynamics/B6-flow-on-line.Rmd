...
raw <- rfun( ~ x, seed=920)
pA <- Znotes::phase_line(raw(x) - 2 ~ x, domain(x=-3.07:-3.05), 
                   nix_dyn=FALSE, 
                   narrows=15, 
                   transform=function(x) x^1.03) 
pB <- Znotes::phase_line(raw(x) - 2 ~ x, domain(x=-1.59:-1.58), 
                   nix_dyn=FALSE, 
                   narrows=15, 
                   transform=function(x) x^1.03) 
...
#| fig-cap: "Pasture dynamics for a sigmoidal consumption function."
#| fig-cap-location: margin
