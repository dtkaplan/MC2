...
```{r echo=FALSE}
#| label: fig-rf-flow-2
#| fig-cap: "The flow field has a vector at each point in state space but we can draw only a few if the plot is to be legible. Zooming in on a region produces more detail."
...
Very near the fixed point, dynamics are approximately linear. We'll return to a quantitative analysis of this in @sec-equilibria. Our objective here is to show that there are several generic types of behavior and that the stability of dynamics near the fixed point has to be one of a handful of different types.
...
We'll use $u$ and $w$ as the dynamical variables in the linear approximation to avoid confusion with the original, nonlinear equations. The linearized dynamics are therefore:
$$\partial_t u = \ \ w\\
\partial_t w = - u$$
