...
Figure \@ref(fig:diff-Euler-separation) shows the difference between the symbolic solution to $\partial_t x = x (1-x)$ and the Euler solution. The figure shows that the Euler solution (with $dt=0.1$) has an approximation error that is  small. The worst case is when $t \approx 3$ at which point it is less than 1.5 parts per million. Another good way to quantify this is the decimal place at which the calculated solutions differ.
