...
pA <- slice_plot(Eco_soln$r(t) ~ t, bounds(t=0:5)) %>% gf_labs(title="Rabbits")
pB <- slice_plot(Eco_soln$f(t) ~ t, bounds(t=0:5)) %>% gf_labs(title="Foxes")
...
traj_plot((Symb_soln(t, A=1/99) - x(t))/Symb_soln(t, A=1/99) ~ t, Euler, bounds(t=0:20))
traj_plot((Symb_soln(t, A=1/99) - x(t))/Symb_soln(t, A=1/99) ~ t, Euler, bounds(t=0:20))
```
