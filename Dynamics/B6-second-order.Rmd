...
1. Basic, pragmatic understanding. This is the level of understanding that you **must have** in order to make sense of the rest of this chapter and @sec-forcing. Here it is:
...
To graph this function, we need to choose appropriate numerical values for $r$ and $b$. Let's set $r=1$. Since $r^2 < 4b$, we must have $\frac{1}{4} < b$: we'll choose $b = 6$ which meets this criterion. @fig-decay-sinusoid1 shows the solution to the differential equation:
...
Let's return to the car-following control system introduced in @sec-eigenvalues. Recall that $x$ was defined to be the distance between two cars and $x_0$ the distance to be maintained. In terms of $y = x - x_0$ the system was $$\partial_{tt} y = - b y\ .$$
