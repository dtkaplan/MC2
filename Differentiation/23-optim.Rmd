...
- Walking up too steep a slope is tiring and slows you down; that is  why hiking trails have switchbacks. When the switchbacks are too shallow, it takes a long time to cover the distance. What's the most efficient angle to enable hikers to get up the hill in the shortest time.
...
#| fig-cap: "Zooming in on the argmax of the objective function. It is important to look at the scale of the vertical axis. Any value of $\\theta$ between about 40 and 50 gives a very close approximation to the maximum."
#| column: margin
...
Solving the optimization problem has prepared us to go out in the field and test the result. Perhaps we will find that the real-world optimum angle is somewhat steeper or shallower than $\theta = 45^\circ$. 
...
With a graph such as @fig-ball-theta, it is easy to find the argmax; common sense carries the day. So it won't be obvious at first why we are going to take the following approach:
