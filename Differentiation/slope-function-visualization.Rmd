...
Segs <- create_segments(sin(x) ~ x, bounds(x=c(-.1,pi)), nsegs=10)
...
Segs <- create_segments(sin(x) ~ x, bounds(x=c(-.1,pi)), nsegs=100)
# gf_segment(yf + yfend ~ x + xend, data = Segs, 
#            color=~slope,size=2, alpha=.9) %>%
...
Segs <- create_segments(exp(x) ~ x, bounds(x=c(-pi,pi)), nsegs=30)
...
Segs <- create_segments(log(x) ~ x, bounds(x=c(.2,pi)), nsegs=30)
ggpubr::ggarrange(
...
gf_segment(y + yend ~ x + xend, data = Segs, 
  ...
  gf_point(0 ~ start, size=1, color="green") %>%
    ...
    gf_refine(scale_color_viridis_c(begin=.3, end=1)) %>%
  gf_theme(theme(legend.position = "none")), ncol = 2)
Segs <- create_segments(x^2 ~ x, bounds(x=c(-pi,pi)), nsegs=30)
ggpubr::ggarrange(
...
gf_segment(y + yend ~ x + xend, data = Segs, 
  ...
  gf_point(0 ~ start, size=1, color="green") %>%
    ...
    gf_refine(scale_color_viridis_c(begin=.3, end=1)) %>%
  gf_theme(theme(legend.position = "none")), ncol=2)
Segs <- create_segments(2*x -4 ~ x, bounds(x=c(-pi,pi)), nsegs=30)
ggpubr::ggarrange(
