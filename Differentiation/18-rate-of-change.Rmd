...
The ***average rate of change*** of $p(t)$ over the interval $t_0 \leq t \leq t_1$ is $$\frac{p(t_1) - p(t_0)}{t_1 - t_0}\ .$$ @sec-dimension-and-units showed that subtraction is legitimate only when the two quantities involved have the same dimension and units. That is the case here. $p(t_1)$ and $p(t_2)$ both have dimension L and miles as the unit. $t_1$ and $t_0$ both have dimension T and hours as the unit.
