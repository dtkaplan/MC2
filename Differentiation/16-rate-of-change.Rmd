...
#| fig-cap: "A line tangent to a the curve at a single point. The slope of this line is the instantaneous rate of change."
Pt <- tibble::tibble(
  t = 25,
  y = water(t),
  baselabel="t[0]",
...
::: {.takenote  data-latex=""}
Here, we write the slope function of $f(t)$ as ${\cal D}f(t)$. That works for this chapter, which deals with functions with only one input. But in general modeling functions have more than one input, for instance $g(x, t)$. To work with slope functions with more than one input, we need to extend the notation a little. We will place a small subscript after ${\cal D}$ to indicate which input we are changing. Thus, there will be two slope functions for $g(x,t)$:
