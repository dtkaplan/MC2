...
::: {#fig-tangent-app}
[![](www/geogebra-tangent-app2.png){width="70%"}](https://www.geogebra.org/m/wTh7KKd3)
...
PB<-   contour_plot(gB(x, y) ~ x + y, bounds(x=c(xB-.25, xB+.25), y=c(yB-.25, yB+.25))) |>
    gradient_plot(gB(x, y) ~ x + y, domain(x=xB + 1*c(-.25,.25)/2, y=yB +1*c(-.25,.25)/2), 
                bounds, npts=3, color="orange3", alpha=1) |>
    gf_refine(coord_fixed())
PzB <- contour_plot(g(x, y) ~ x + y, bounds(x=xB + 1*c(-.25,.25), y=yB +1*c(-.25,.25))) |>
...
bounds = bounds(x=c(xC-.1, xC+.1), y=c(yC-.1, yC+.1))
...
The point here is to show that two-numbers are sufficient to dictate the orientation of a plane. In terms of @fig-hold-book these are 1) the amount that one hand is raised relative to the other and 2) the angle of rotation around the hand-to-hand axis. 
