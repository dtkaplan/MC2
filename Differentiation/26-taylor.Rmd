...
```
To judge from this plot, $\sin(0) / 0 = 1$.
:::
...
slice_plot(sin(x) / x^3 ~ x, 
  ...
  gf_refine(scale_y_log10())
```
Zooming in around the division by zero. Top: The graph of $\sin(x)/x$ versus $x$. Bottom: The graph of $\sin(x)/x^2$. The vertical scales on the two graphs are utterly different.
:::
...
`r insert_exercises("Differentiation", "taylor polynomials")`
