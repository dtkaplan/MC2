...
Much earlier [records from Babylonian clay tablets](https://en.wikipedia.org/wiki/Babylonian_mathematics) show that as long ago as 2000-1600 BC, mathematicians were factoring quadratics. 
...
A $n$th-order polynomial can have up to $n-1$ critical points that it wriggles among. A 7-th order polynomial is shown here in which there are six local maxima or minima alternatingly.
:::

@fig-wriggly-polynomial shows a 7th order polynomial---that is, the highest-order term is $x^7$. In the wriggly domain in @fig-wriggly-polynomial, there are six argmins or argmaxes. In one of the tail domains the function value heads off to $\infty$, in the other to $-\infty$. This is a inescapable feature of all odd-order polynomials: 1, 3, 5, 7, ...
...
The run-away behavior of polynomials provides no insurance against wild, misleading extrapolation of model formulas. Such insurance is provided instead by sigmoid, gaussian, and sinusoid functions, as well as more modern constructions such as "smoothers," "natural splines," and the "wavelets" originating in fractal theory.



## Indeterminate forms

Let's return to an issue that has bedeviled mathematicians for millenia and mislead even the famous Brahmagupta. This is the question of what it means to divide zero by zero or, more generally, to divide any number by zero.
...
The sinc() function (pronounced "sink") is important today, in part because of its role in converting discrete-time measurements (as in an mp3 recording of sound) into continuous signals. So there really are occasions that call for evaluating it at an input of zero.
...
On the other hand, $\sin(0) = 0$ as well, so the sinc function evaluated at zero involves 0/0. This quotient is called an ***indeterminate form***. The logic is this: Suppose we assume that $0/0 = b$ for some number $b$. then $0 = 0 \times b = 0$. So any value of $b$ would do; the value of $0/0$ is "indeterminate."
...
Both $\sin(x)/x$ and $\sin(x) / x^3$ involve a divide by zero when evaluated at $x=0$. Both are indeterminate forms 0/0 at $x=0$. But the graph of $\sin(x) / x^3$ (see @lst-sinc2) is not will behaved.  $\sin(x) / x^3$ does not have any particular value at $x=0$; instead, it has an asymptote. 
...
But using the sloppy, non-evanescent notation is convenient for starting to understand why constructions like
