...
#| out-width: "33%"
#| fig-cap: "Zooming in on the neighborhoods of A, B, and C in @fig-whole-plot shows a simple, almost planar, local landscape."
PzC
PzA
PzB
```
...
In @fig-facet-plot, the facets tangent to the original surface at A, B, and C are displayed. Comparing Figures \@ref(fig:zoomed-plot) and \@ref(fig:facet-plot) you can see that each facet has the same orientation as the surface; the contours face in the same way. 


```{r echo=FALSE, fig.show="hold"}
#| label: fig-facet-plot
#| out-width: "33%"
#| fig-cap: "The facets around the points are linear functions, each aligned with the contours near that point in @fig-whole-plot."
PC
PA
PB
```
