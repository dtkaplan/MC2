...
In many modeling situations with a single input, you can get very close to a good modeling function $f(x)$ by selecting one of ***eight simple shapes***, shown in @fig-eight-simple-shapes. 

```{r echo=FALSE}
#| label: fig-eight-simple-shapes
#| out-width: "40%" 
#| fig-cap: "The ***eight simple shapes***, locally, of functions with one input. (See  @sec-local-approximations.)"
#| fig-cap-location: margin
g <- makeFun(a + b*x + c*x^2 ~ x, a=-1, b=2, c=1)
P1 <- slice_plot(-g(x, c=0) ~ x, domain(x=c(-3,1)), size=2  ) %>%
  gf_labs(y="Output", x="Input", title="(A) downward sloping line")
P2 <- slice_plot(g(x, c=0) ~ x, domain(x=c(-3,1)), size=2  ) %>%
  gf_labs(y="Output", x="Input", title="(B) upward sloping line")
P3 <- slice_plot(g(x) ~ x, domain(x=c(-3,-1)), size=2) %>%
  gf_labs(y="Output", x="Input", title="(C) downward sloping, concave up; steep then shallow")
P4 <- slice_plot(g(x) ~ x, domain(x=c(-1,1)), size=2) %>% 
  gf_labs(y="Output", x="Input", title="(D) upward sloping, concave up; shallow then steep")
P5 <- slice_plot(-g(x) ~ x, domain(x=c(-3,-1)), size=2) %>% 
  gf_labs(y="Output", x="Input", title="(E) upward sloping, concave down; steep then shallow")
P6 <- slice_plot(-g(x) ~ x, domain(x=c(-1,1)), size=2) %>%
  gf_labs(y="Output", x="Input", title="(F) downward sloping, concave down; shallow then steep")
P7 <- slice_plot(g(x) ~ x, domain(x=c(-3,1)), size=2) %>%
  gf_labs(y="Output", x="Input", title="(G) local minimum")
P8 <- slice_plot(-g(x) ~ x, domain(x=c(-3,1)), size=2) %>% 
...
if (knitr::is_html_output()) {
  ...
  knitr::kable(Zcalc::Body_fat %>% head(10))
}
```
