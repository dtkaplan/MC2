...
::: {.example data-latex=""}
The expression  $\partial_x x^3$ is the same as $\partial_x \left[\strut x\  x^2\right]$. Since we already know $\partial_x x$ (it's 1) and $\partial_x x^2$ (it's $2x$) let's apply the product rule to find $\partial_x x^3$:
...
A function composition, as described in Section @sec-function-composition, involves inserting the output of one function (the "interior function") as the input of the other function (the "exterior function"). As we so often do, we'll be using pronouns a lot. A list might help keep things straight:
...
Such changes in logarithms are encountered in fields such as economics or finance, where it's common to consider the logarithm of the economic quantity to render changes as percent of the whole. 
...
r"(The constant multiplier rule)" = r"( )",
  r"(The linear combination rule)" = r"( )",
  r"(+The product rule+)" = r"( )",
  r"(The chain rule)" = r"(You could argue that you'll need the chain rule, too, to find $\partial_t e^{-t}$. But before you get there, you'll need the product rule.)",
  r"(No rule needed, it's so basic.)" = r"( )",
  random_answer_order=FALSE
)
```
