...
Not until the 1850s was a workable theory of vanishing $h$ accepted by the mathematics community. This "theory of limits" is sometimes taught as the centerpiece of introductory calculus even though it has little connection to the uses of calculus.
...
For functions with multiple inputs, for instance, $h(x,y,z)$, differentiation can be done with respect to any input. Leibniz's notation might possibly be used to indicate which is the with-respect-to input; the three derivatives of $h()$ would be written $df/dx$ and $df/dy$ and $df/dz$. However, mathematical notation did not go in this direction. Instead, for functions with multiple inputs, the three derivatives are most commonly written $\partial f/\partial x$ and $\partial f/partial y$, and $\partial f/\partial z$. In the expression, $\partial f/\partial y$, the symbol $\partial$ is pronounced "partial," and $\partial f/\partial y$ is called a "partial derivative" (as are $\partial f/partial x$ and $\partial f \partial z$).
...
It has been more than 300 years since Leibniz's death. At this point calculus is so we will established that we don't need the notation to remind us that a derivative is "a little bit of $f$ divided by a little bit of $x$." 
...
To read calculus fluently, you will have to recognize each of these notations. 
