...
In this chapter, we will elaborate on ***continuity***, one of the ideas introduced in Section @sec-word-descriptions, and use the concept of continuity to characterize functions in a new way: their ***smoothness***. 
...
Common sense suggests that a change of 0.002 cc in the amount of water---that is  a small fraction of a drop, 2 cubic millimeters of volume, is not going to lead to a qualitative change in output. So you might prefer to use a sigmoidal function as your model rather than a Heaviside function. 
...
HH <- antiD(H(x) ~ x)
HHH <- antiD(HH(x) ~ x)
pA <- slice_plot(HHH(x) ~ x, domain(x=-0.1:0.1), npts=1000) 
```
